...
<form action="upload_to_uploads_subfolder.php" method="post" enctype="multipart/form-data">
  ...
  if($query->num_rows > 0){
      while($row = $query->fetch_assoc()){
          $imageURL = 'uploads/subfolder/'.$row["file_name"];
  ?>
      <img src="<?= $imageURL; ?>" />
  <?php }} ?>
</body>
...
// basename($_FILES["file"]["name"]) = 'hero-bg.png';
...
if(isset($_POST["submit"]) && !empty($_FILES["file"]["name"])){
    ...
    $allowTypes = array('jpg','png','jpeg','gif','pdf', 'xls');
    if (!file_exists($targetFilePath)) {
        if(in_array($fileType, $allowTypes)){
            ...
            $statusMsg = "Sorry, only JPG, JPEG, PNG, GIF, XLS, & PDF files are allowed to upload." . $backlink;
        }
    }else{
